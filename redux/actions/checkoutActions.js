...
export const getSessionPagamento = () => (dispatch) => {
  axios
    .get(`${API}/${versao}/api/pagamentos/session`)
    .then((response) => {
      ...
      console.log("SEEENDER", senderHash);
...
export const novoPedido =
  ...
  (dispatch) => {
    axios
      .post(
        `${API}/${versao}/api/pedidos?loja=${loja}`,
        {
          carrinho,
          entrega: {
          ...
          pagamento: {
            ...
            endereco: {
              local: !form.dadosEntregaIgualDadosCobranca
                ? form.dadosCobranca.local
                : form.local,
              numero: !form.dadosEntregaIgualDadosCobranca
                ? form.dadosCobranca.numero
                : form.numero,
              complemento: !form.dadosEntregaIgualDadosCobranca
                ? form.dadosCobranca.complemento
                : form.complemento,
              bairro: !form.dadosEntregaIgualDadosCobranca
                ? form.dadosCobranca.bairro
                : form.bairro,
              cidade: !form.dadosEntregaIgualDadosCobranca
                ? form.dadosCobranca.cidade
                : form.cidade,
              estado: !form.dadosEntregaIgualDadosCobranca
                ? form.dadosCobranca.estado
                : form.estado,
              CEP: !form.dadosEntregaIgualDadosCobranca
...
export const pagarPedido =
  (id, token, senderHash, cb = null) =>
  (dispatch) => {
    axios
      .post(
