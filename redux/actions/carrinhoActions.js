...
export const fetchVariacoesCarrinho =
  (id, produto, idxCarrinho) => (dispatch) => {
    axios
      .get(
        `${API}/${versao}/api/variacoes/${id}?produto=${produto}&loja=${loja}`
      )
      .then((response) =>
        dispatch({
...
export const calcularFrete = (cep, carrinho) => (dispatch) => {
  axios
    .post(`${API}/${versao}/api/entregas/calcular`, { cep, carrinho })
    .then((response) =>
      dispatch({
