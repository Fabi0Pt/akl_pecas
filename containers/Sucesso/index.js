...
class SucessoContainer extends Component {
  renderBoleto() {
    ...
    console.log("AQUIII", pagamento.auth[0].paymentLink);
    return (
      <div>
        <p>
        ...
        <a
          ...
          href={pagamento.auth[0].paymentLink}
          target="_blank"
  ...
  renderCartao() {
    return (
      <div>
        <p>
          O pagamento está sendo processado e você receberá uma confirmação pelo
          email em breve. Obrigado pelo pedido.
        </p>
        <br />
      </div>
    );
  ...
  render() {
    ...
    return (
      <div className="Sucesso-Container container">
        {pagamento &&
        pagamento.auth &&
        pagamento.auth.length > 0 &&
        !pagamento.auth[0].error &&
        pagamento.auth
          ? this.renderSucesso()
          : this.renderErro()}
      </div>
